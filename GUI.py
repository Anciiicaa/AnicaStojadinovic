...
root = tk.Tk()
root.title("Unos podataka")
root.geometry("600x400")  # Promenite dimenzije prema vašim potrebama
...
style = ttk.Style()
style.configure('TEntry', font=('Helvetica', 14))  # Promenite veličinu fonta prema vašim potrebama
...
batch_number_var = tk.Entry(root, textvariable=batch_number_var)
batch_number_var.grid(row=6, column=1)
# Kreiranje i postavljanje labela i input polja za količine
kolicina_labels = ["Kolicina pregledanih delova", "Kolicina NOK", "Kolicina OK"]
for i in range(3):
...
# Postavite naziv servera i bazu podataka
    ...
    batch_number = broj_delova_var.get()
    ...
    sql_query = """INSERT INTO GP12 (Smena, Datum, Operater, Workstation, Project, PartNumber, BatchNumber, QuantityInspected, QuantityNOK, QuantityOK, Defect, DefectDescription)
                   VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)"""

    # Izvršavanje SQL upita sa vrednostima
    cursor.execute(sql_query, (smena, datum, operater, stanica, project, part_number, batch_number, kolicina_pregledanih, kolicina_nok, kolicina_ok, defect, defect_description))
...
tk.Button(root, text="Sačuvaj", command=dugme_sacuvaj_klik).grid(row=20, columnspan=2)
